...
const resources = {
  ...
  sessions: { table: 'sessions', order: 'started_at DESC, session_id', fields: ['session_id', 'job_posting_id', 'session_type', 'targeted_weakness', 'started_at', 'ended_at'] },
...
export function createDataRouter(db) {
  ...
  router.patch('/profile', asyncRoute(async (req, res) => {
    const body = req.body;
    if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(k => !['name', 'targetRoles', 'jobPosting', 'resume'].includes(k)) || typeof body.name !== 'string' || typeof body.targetRoles !== 'string' || (body.jobPosting !== null && typeof body.jobPosting !== 'string')) throw new HttpError(400, 'Invalid interview profile.');
    await ensureUser(db, req.user);
