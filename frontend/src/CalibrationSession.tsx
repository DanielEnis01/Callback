...
// A single SDK result is not a useful baseline. Require a few valid readings
// for every signal before saving, while still allowing the SDK to take longer
// for metrics such as EDA that initialize late.
...
function getBaselineReadiness(samples: CalibrationSample[], mediaPipeSamples: Parameters<typeof buildMediaPipeBaseline>[0]) {
  ...
  const mediaPipeReady = buildMediaPipeBaseline(mediaPipeSamples) !== null;
  const items = [
    ...
    { key: "eda", label: "Skin conductance", count: validCount(samples, (s) => s.eda) },
    { key: "seat", label: "Seat motion", count: validCount(samples, (s) => s.micromotionSeat) },
    { key: "knees", label: "Knee motion", count: validCount(samples, (s) => s.micromotionKnees) },
    { key: "blinks", label: "Blink rate", count: samples.filter((sample) => sample.blinkDetected != null).length },
    { key: "posture", label: "Posture", count: mediaPipeSamples.filter((s) =>
      [s.headYaw, s.headPitch, s.headRoll, s.shoulderTiltDeg, s.torsoLeanDeg,
        s.shoulderDistance, s.poseMovementRate].every((v) => typeof v === "number" && Number.isFinite(v))).length },
  ].map((item) => ({
    ...item,
    ready: item.key === "posture" ? mediaPipeReady : item.count >= MIN_VALID_READINGS,
  }));
...
export const CalibrationSession: FC<CalibrationSessionProps> = ({ onDone, onCancel }) => {
  ...
  const readingStarted = phase === "recording" && !readingInstructionsOpen && !gazeCalibrationOpen;
  ...
  const baselineReadiness = useMemo(
    () => getBaselineReadiness(samplesRef.current, mediaPipeSamplesRef.current),
    [sampleTick, samplesRef, mediaPipeSamplesRef],
  );

  const missingSignals = baselineReadiness.items.filter((item) => !item.ready).map((item) => {
    let reason: string;
    if (item.key === "posture") {
      reason = mediaPipe.error || (mediaPipe.status === "loading"
        ? "The posture models are still loading."
        : "Waiting for five complete head, shoulder, torso, and movement readings. Keep your upper body visible.");
    } else {
      ...
      reason = error || pipelineHint || (status === "error" ? "The SDK measurement engine reported an error. Cancel and restart calibration." : "") ||
        ...
        (item.key === "stress" ? "No complete HRV readings yet. Stress is calculated from heart-beat intervals and depends on a usable pulse signal."
          ...
          : item.key === "eda" ? "No usable skin conductance readings yet. EDA requires its extra SDK model and takes over 35 seconds to produce its first result."
          : item.key === "seat" || item.key === "knees" ? "No usable micromotion readings yet. These signals require extra SDK models and appropriate body framing."
          : item.key === "blinks" ? "No blink detection readings yet. Keep your eyes visible."
    ...
    if (!readingStarted && !error && !pipelineHint) reason = "Baseline collection has not started. Complete the camera/posture check and select Begin reading. " + reason;
  ...
  useEffect(() => {
    if (
      ...
      status === "running" && mediaPipe.status === "ready" &&
  ...
  }, [phase, elapsed, baselineReadiness.ready, readingInstructionsOpen, gazeCalibrationOpen, facePausedForTooLong, status, mediaPipe.status, samplesRef, mediaPipeSamplesRef]);
      ...
      <header className="h-14 shrink-0 flex items-center justify-between border-b border-white/12 px-6">
        ...
        {phase === "recording" && (
          <div className="flex items-center gap-4 text-[13px] text-white/50">
            <span className="tabular-nums">{clock} / at least 00:40</span>
        ...
        <div className="flex-1 min-h-0 flex items-center justify-center p-8">
          <div className="max-w-lg flex flex-col gap-6">
            ...
            <p className="text-[15px] leading-relaxed text-white/60" style={{ fontWeight: 300 }}>
              ...
              good and holds steady for a couple seconds, a reading starts automatically. It runs for at least
              40 seconds and continues until every baseline signal has enough valid data.
            </p>
          ...
          <div className="flex-1 min-h-0 flex flex-col gap-4 p-4">
            ...
            <div className="shrink-0 border border-white/12 p-5">
              <span className="text-[11px] uppercase tracking-[0.16em] text-white/40">
                Read aloud and scroll at your own pace until every baseline signal is ready
              </span>
              ...
              {baselineReadiness.items.map((item) => (
                <div key={item.key} className="bg-black px-3 py-2 flex items-center justify-between gap-2">
                  ...
                  <span className={`shrink-0 text-[10px] tabular-nums ${item.ready ? "text-white" : "text-white/35"}`}>
                    {item.ready ? "Ready" : `${item.count}/${item.key === "posture" ? 5 : MIN_VALID_READINGS}`}
                  </span>
                </div>
              ))}
            </div>
              ...
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80 p-6">
                <div className="w-full max-w-lg border border-white/20 bg-black p-6 shadow-2xl">
                    ...
                    {mediaPipe.status === "error"
                      ...
                      : mediaPipe.status === "loading"
                        ...
                        : mediaPipe.lookingAtCamera
                          ? gazeCalibrationDone ? "Ready to collect posture readings" : "Great — hold steady for a moment…"
                  ...
                  <button
                    ...
                    disabled={mediaPipe.status !== "ready"}
                  ...
                  <button onClick={onCancel} className="ml-4 text-[13px] text-white/60">Cancel</button>
                </div>
              </div>
            )}
            ...
            {readingInstructionsOpen && !gazeCalibrationOpen && (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80 p-6">
                <div className="w-full max-w-lg border border-white/20 bg-black p-6 shadow-2xl">
                  ...
                  <p className="mt-4 text-[15px] leading-relaxed text-white/70" style={{ fontWeight: 300 }}>
                    Please read the quotes aloud until every signal card says Ready and the minimum recording
                    time has passed. You can repeat the quotes if you reach the end.
                  </p>
