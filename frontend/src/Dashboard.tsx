...
const SettingsView: FC<{ onLogout: () => void }> = ({ onLogout }) => {
  const [position, setPosition] = useState(() => getInterviewProfile()?.targetRoles || "Software Engineer");
  const saveTarget = (targetRoles: string, resume: ResumeDocument | null = getReadyResume()) => {
    const profile = getInterviewProfile();
    saveInterviewProfile({
      name: profile?.name ?? "",
      targetRoles,
      jobPosting: profile?.jobPosting ?? null,
      resume: resume ? { name: resume.name, size: resume.size, documentId: resume.documentId } : null,
    });
  };
  ...
  return (
    <>
      ...
      <SectionCard title="Resume">
        <ResumeUpload onReadyChange={(resume) => { if (resume) saveTarget(position, resume); }} />
      </SectionCard>
      ...
      <SectionCard title="Target job position">
        ...
        <div className="flex flex-wrap gap-2">
          {Array.from(new Set([...jobPositions, position])).map((p) => {
            const on = p === position;
            return (
              <button
                key={p}
                onClick={() => { setPosition(p); saveTarget(p); }}
