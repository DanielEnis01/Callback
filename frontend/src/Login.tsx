...
const inputClass = "w-full bg-white/[0.04] border border-white/15 text-white text-[15px] placeholder:text-white/30 px-4 py-3 rounded-none outline-none transition-colors focus:border-white/60 disabled:opacity-50";
...
export default function Login() {
  ...
  return (
    <main className="flex flex-col justify-center px-6 py-12 sm:px-12 lg:w-[48%] lg:min-h-screen bg-black">
      <form className="flex flex-col gap-4 max-w-[420px] w-full mx-auto lg:mx-0" onSubmit={submit} aria-busy={auth.busy}>
        <p className="mb-5 text-[13px] text-white/50">Callback.</p>
        <h1 className="text-[34px] leading-tight text-white tracking-tight font-extrabold">
          {needsUsername ? "Your username" : mode === "signin" ? "Sign in" : mode === "signup" ? "Sign up" : "Reset password"}
        </h1>
        <p className="text-[13px] leading-relaxed text-white/50 mb-2">
        ...
        <fieldset disabled={auth.busy} className="flex flex-col gap-4 min-w-0">
          {(mode === "signup" || needsUsername) && <div className="flex flex-col gap-1.5">
            <label htmlFor="username" className="text-[13px] text-white/70">Username</label>
          ...
          {!needsUsername && mode !== "reset" && <div className="flex flex-col gap-1.5">
            <label htmlFor="password" className="text-[13px] text-white/70">Password</label>
          ...
          <button type="submit" disabled={unavailable || auth.busy} className="mt-1 w-full bg-white text-black text-[14px] font-semibold tracking-wide px-7 py-3 transition-opacity active:opacity-70 disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          {!needsUsername && mode !== "reset" && <>
            ...
            <button type="button" onClick={googleSignIn} disabled={unavailable || auth.busy} className="w-full flex items-center justify-center gap-3 border border-white/15 text-white text-[14px] font-medium px-7 py-3 transition-colors hover:border-white/40 disabled:opacity-40 disabled:cursor-not-allowed">
